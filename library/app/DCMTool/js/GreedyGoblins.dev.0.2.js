/*
  global
  string of DOM template
*/
 var string_DOMTemplate = {
   defaultNavTabsID: ["char", "meas", "func"],
   dropAreaTable:`
     <table class='table'>
      <button type="button" id="{{id-btn-parse}}">parse</button>
       <thead>
         <tr>
           <th>#</th>
           <th>name</th>
   ...
   `,
   navTabsItem:{
     char: '<li role="presentation" class="active"><a href="#id-content-char" data-toggle="tab"> CHARACTERISTIC<span class="badge">0</span></a></li>',
     meas: '<li role="presentation"><a href="#id-content-meas" data-toggle="tab"> MEASUREMENT<span class="badge">0</span></a></li>',
     func: '<li role="presentation"><a href="#id-content-func" data-toggle="tab"> FUNCTION<span class="badge">0</span></a></li>',
   },
   navTabsContentDivItem:{
...
if (window.HTMLElement){
  HTMLElement.prototype.GreedyGoblins = function(){
    if (this){ //.tagName == "DIV"
      let root = this;
      //this.id = "id-greedygoblins-"+performance.now();
      this.fileList = [];
      this.a2lFile = null;
      this.hexFiles = [];
      this.initialize = function(){
        ...
        // 1. dom of droparea
        let dom_dropArea = document.createElement("div");
        dom_dropArea.id="id-dropArea";
        dom_dropArea.style = "width:100%;height:300px;background-color:#EEE;border-style:dashed;border-color:#CCC";
        dom_dropArea.className = "css-div-droparea-fileinput";
        this.append(dom_dropArea);
        $(dom_dropArea).append(string_DOMTemplate["dropAreaTable"].replace("{{id-btn-parse}}", "id-btn-parse"+root.id));

        dom_dropArea.addEventListener("dragover", function(e){
          e.preventDefault();

        })
        dom_dropArea.addEventListener("drop", function(e){
          e.preventDefault();
          let files = e.dataTransfer.files;
          for (let file of files){
            if (file.name.toUpperCase().match("A2L") && (!root.a2lFile)){ //&& file.name.match("a2l")
              ...
              fr.onload = function(e){
                ...
                if (!alreadyExist(file, root.fileList)){
                  ...
                  root.a2lFile = _tempFileObj;

                  // add new row into droparea table
                  let tbody_droparea = $(root).find(".css-div-droparea-fileinput tbody")[0];
                  let string_row = string_DOMTemplate["dropAreaTableItem"]
                    .replace("{{checkbox}}", "<input type='checkbox' value='a2lFile'>")
                    .replace("{{name}}", file.name)
                    .replace("{{type}}", "A2L")
                    .replace("{{size}}", file.size);

                  $(tbody_droparea).append(string_row);
              ...
              fr.onload = function(e){
                ...
                if (!alreadyExist(file, root.fileList)){
                  ...
                  root.hexFiles.push(_tempFileObj);

                  // add new row into droparea table
                  let tbody_droparea = $(root).find(".css-div-droparea-fileinput tbody")[0];
                  let string_row = string_DOMTemplate["dropAreaTableItem"]
                    .replace("{{checkbox}}", "<input type='checkbox' value='hexFile"+(root.hexFiles.length-1)+"'>")
                    .replace("{{name}}", file.name)
                    .replace("{{type}}", "HEX")
                    .replace("{{size}}", file.size);
                  $(tbody_droparea).append(string_row);
        ...
        // 2. tabs of contents
        const config_navTabs = string_DOMTemplate["defaultNavTabsID"];

        let dom_tabs_nav = document.createElement("ul");
        dom_tabs_nav.className = "nav nav-tabs";

        let dom_tabs_content = document.createElement("div");
        dom_tabs_content.className = "tab-content";

        for (let i=0; i<config_navTabs.length; i++){
          let dict = string_DOMTemplate["navTabsItem"];
          $(dom_tabs_nav).append(dict[config_navTabs[i]]);

          let css_contentItem = "tab-pane fade";
          if (i == 0) css_contentItem = "tab-pane fade in active";
          let contentItem = document.createElement("div");
          contentItem.className = css_contentItem;
          contentItem.id = "id-content-"+config_navTabs[i];
          dom_tabs_content.append(contentItem);
        }
        this.append(dom_tabs_nav);
        this.append(dom_tabs_content);
        $("#id-btn-parse"+root.id).on("click", function(){
          let selectedA2L = null;
          let selectedHEX = [];
          $(root).find("input").each(
            function(){
              if ($(this).attr("type") == "checkbox" && $(this)[0]["checked"] == true){
                if ($(this).val() == "a2lFile"){
                  selectedA2L = true;
                }
                else {
                  const str = $(this).val()
                  selectedHEX.push(parseInt(str.substr(str.length-1, 1)));
                }
              }
            }
          );
          if (selectedA2L){
            root.show(root.fetch({
              a2l: root.a2lFile,
              hex: root.hexFiles[selectedHEX[0]],
              show: true
            }));
      ...
      this.show = function(result){
        let tabsID = "char";
        let div = $("#id-content-"+tabsID);
        let ul = document.createElement("ul");
        div.append(ul);
        console.log(result);
        for (const c of result["CHARFound"]){
          $(ul).append("<li>"+c["name"]+"</li>")
        }
      }
